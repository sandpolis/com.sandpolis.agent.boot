...
impl MainUI {
    pub fn new(resolutions: Vec<(usize, usize)>) -> Option<MainUI> {
        ...
        let gop = bt.locate_protocol::<GraphicsOutput>()?.expect("Warnings encountered while opening GOP");
...
impl Textline {
    pub fn render(&self) {
        ...
        layout.append(&[self.font], &TextStyle::with_user_data(text, 35.0, 0, 10u8));
